...
<p>
    Класс [[\yii\base\Model]] является также базовым классом для более продвинутых моделей с дополнительной функциональностью, таких как Active Record.
</p>
<h2>
    Атрибуты
</h2>
...
<p>
    ...
    Переопределенный метод возвращает массив, в котором ключом является атрибут модели, а значение является label'ом.
...
<p>
    Массовое присваивание по-прежнему доступно по умолчанию:<br />
    <?php
    highlight_string("<?php
\$user = User::find(42);
\$data = [
