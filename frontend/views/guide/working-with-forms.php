...
<p>
    В приведенном выше коде, [[yii\widgets\ActiveForm::begin()|ActiveForm::begin()]] не только создает экземпляр формы, но и знаменует собой начало формы. Все содержание, помещенное между [[yii\widgets\ActiveForm::begin()|ActiveForm::begin()]] и [[yii\widgets\ActiveForm::end()|ActiveForm::end()]] будет обернуты в тег <form>. Как и в любом виджете, вы можете указать несколько вариантов того, как виджет должен быть настроен, передав массив в метод begin. В этом случае дополнительный css класс и параметр id передаются для использования в открывающемся теге <form>.
</p>
<p>
...
<p>
    Это создаст все <label>, <input> и другие теги в соответствии с шаблоном, определяемым в поле формы. Чтобы добавить эти теги самому, вы можете использовать вспомогательный класс HTML. Следующий код эквивалентен коду выше:<br />
    <?php
    highlight_string("<?php
...
<blockquote>
    <p>
        Совет: для изменения стиля необходимых полей со звездочкой, можно использовать следующий CSS:
    </p>
    <?php
    highlight_string("<?php
