...
<hr />
<p>
    Yii приложения полагаются на компоненты для выполнения наиболее распространенных задач, таких как подключение к базе данных, маршрутизация запросов браузера и обработка сессий. Поведение этих компонентов в своем приложении можно регулировать с помощью настроек в конфиг-х файлах. Большинство компонентов имеют разумные значения по умолчанию, так что маловероятно, что вы будете делать много конфигурации. Тем не менее, есть некоторые обязательные параметры конфигурации, которые вам придется установить, например, соединения с базой данных.
...
<p>
    ...
    Для веб-приложений, файл начальной загрузки обычно index.php.
...
<hr />
<p>
    Экземпляр приложение настраивается, когда он создается в файле начальной загрузки.
    Конфигурация обычно хранится в файле РНР, хранящейся в директории приложения '/config'. Файл имеет следующую начальную структуру:<br />
    <?php
    highlight_string("<?php
return [
    'id' => 'applicationId',
...
<p>
    ...
    В зависимости от типа приложения, вы можете настроить свойства для [[yii\web\Application]] или [[yii\console\Application]] классов.
    Оба класса расширяют [[yii\base\Application]].
</p>
<p>
    Обратите внимание, что вы можете настроить не только public свойства класса, но и любое свойство доступно через сеттер.
    ...
    Возможность настройки свойств через сеттеры доступен любому классу, который наследуется от [[yii\base\Object]], так как почти каждый класс в рамках Yii наследуется от [[yii\base\Object]].
</p>
<h2>
...
<p>
    В приведенном выше коде настраиваются четыре компонента: cache, user, errorHandler, log. Каждый ключ яв-ся ID компонента.
    Значениями яв-ся подмассивы, используемые для настройки этого компонента.
...
<p>
    ...
    Чтобы настроить один из них, ключ с именем 'class' можно не указывать, чтобы использовать класс Yii по умолчанию для этого компонента.
    Вы можете проверить registerCoreComponents() - метод приложения, чтобы получить список идентификаторов компонентов и соответствующих классов.
</p>
<p>
    Обратите внимание, что Yii достаточно умен, чтобы настроить компонент, только когда он фактически используется: ​​например, если вы настроите компонент кэша в файле конфигурации, но никогда не будете его использовать в коде, то ни одного экземпляра этого компонента не будет создано и не будет потрачено время на его настройку.
</p>
<h2>
...
<p>
    ...
    Например, если вы хотите изменить класс, используемый для всех виджетов LinkPager без указания класса для каждого использования виджета, вы можете сделать следующее:<br />
    <?php
    highlight_string("<?php
\Yii::\$objectConfig = [
